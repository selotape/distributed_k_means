 ...
 */
@Since("1.5.0")
class MyKMeansModel private[ml](
                                @Since("1.5.0") override val uid: String,
                                private[clustering] val parentModel: MLlibKMeansModel)
  extends Model[MyKMeansModel] with KMeansParams with GeneralMLWritable
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): MyKMeansModel = {
    val copied = copyValues(new MyKMeansModel(uid, parentModel), extra)
   ...
   *
   * For [[MyKMeansModel]], this does NOT currently save the training [[summary]].
...
private class InternalKMeansModelWriter extends MLWriterFormat with MLFormatRegister {
  ...
  override def write(path: String, sparkSession: SparkSession,
    ...
    val instance = stage.asInstanceOf[MyKMeansModel]
...
@Since("1.6.0")
object MyKMeansModel extends MLReadable[MyKMeansModel] {

  @Since("1.6.0")
  override def read: MLReader[MyKMeansModel] = new KMeansModelReader

  @Since("1.6.0")
  override def load(path: String): MyKMeansModel = super.load(path)
  ...
  private class KMeansModelReader extends MLReader[MyKMeansModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[MyKMeansModel].getName

    override def load(path: String): MyKMeansModel = {
      ...
      val model = new MyKMeansModel(metadata.uid, new MLlibKMeansModel(clusterCenters))
...
class MyKMeans @Since("1.5.0")(
                               @Since("1.5.0") override val uid: String)
  extends Estimator[MyKMeansModel] with KMeansParams with DefaultParamsWritable {
  ...
  @Since("2.0.0")
  override def fit(dataset: Dataset[_]): MyKMeansModel = instrumented { instr =>
    ...
    val model = copyValues(new MyKMeansModel(uid, parentModel).setParent(this))
